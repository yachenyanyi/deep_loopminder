...
async def init_postgres_checkpointer():
    """初始化PostgreSQL checkpointer，失败时回退到内存存储"""
    ...
    async with postgres_checkpointer_lock:
        ...
        try:
            ...
            print(f"⚠️ PostgreSQL checkpointer 连接失败: {e}")
            print("⏳ 回退到内存存储 (InMemorySaver)")
...
async def init_postgres_store():
    """初始化PostgreSQL store，失败时回退到内存存储"""
    ...
    async with postgres_store_lock:
        ...
        try:
            ...
            print(f"⚠️ PostgreSQL store 连接失败: {e}")
            print("⏳ 回退到内存存储 (InMemoryStore)")
